    ...
    <div id="site-content" style="display: none;">
      ...
      <div class="w3-sidebar w3-bar-block">
       ...
       <!--Cadastrar-->
        ...
        <a href="#" class="w3-bar-item w3-button" style="text-decoration: none;">Cancelar &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      ...
      <form id="form1" style="display: none;" method="post">
        <div class="form-row">
          <div class="form-group col-md-6">
            ...
            <input type="text"  name="nomeProduto" class="form-control" id="name" placeholder="Insira o nome" required>
          </div>
          <div class="form-group col-md-6">
        ...
        <div class="form-row">
          <div class="form-group col-md-6">
            ...
            <input type="text" name="codigoProduto" class="form-control" id="code" placeholder="Insira o código" required>
          </div>
        </div>
        ...
        <div class="form-row">
          <div class=" form-group col-md-6">
            ...
            <select class="form-control" id="category" required name="categoriaProduto">
          ...
          <div class="form-group col-md-6 dropdown">
            ...
            <select class="form-control" id="unit" required name="unidadeProduto">
        ...
        <div class="form-row">
          <div class="form-group col-md-8">
            ...
            <div class="custom-file">
              <input type="file" class="custom-file-input" id="formFile" name="fotoProduto">
              <label class="custom-file-label" for="formFile">Escolha um arquivo</label>
            </div>
          </div>
        </div>
            <input type="submit" value="Cadastrar"  class="w3-bar-item w3-button " >
      </form>
    </div>
///
//Pessoa
    ...
    <div class="col-md-10 ml-sm-auto">
     <form id="form2" style="display: none;" method="post">
        <div class="form-row">
          <div class="form-group col-md-6">
            <label for="name">Nome</label>
            <input type="text" name="nomePessoa" class="form-control" id="name" placeholder="Insira o nome" required>
          </div>
          <div class="form-group col-md-6">
        ...
        <div class="form-row">
          <div class="form-group col-md-6">
            ...
            <input type="text" name="codigoPessoa" class="form-control" id="code" placeholder="Insira o código" required>
          </div>
          <div class="form-group col-md-6 dropdown">
            <label for="unit">Tipo de usuário</label>
            <select class="form-control" id="unit" required  name="tipoPessoa">
      ...
      <form id="form3" style="display: none;" method="post">
        <div class="form-row">
          <div class="form-group col-md-6">
            ...
            <input type="text" class="form-control" id="name" placeholder="Insira a unidade de medida" required  name="unidademedida">
          </div>
          <div class="form-group col-md-6">
      ...
      <form id="form4" style="display: none;">
        <div class="form-row">
          <div class="form-group col-md-6">
            ...
            <input type="text" class="form-control" id="name" placeholder="Insira a categoria" required name="categoria">
          </div>
...
<!--Cadastro de produto e incorporação em banco de dados-->
  <?php
  ...
  try{
    ...
    if ((trim($unidademedida) == "")) {
      ...
      include("conexaobanco.php");
        $stmt = $pdo->prepare("select * from unidademedidatcc where unidademedida = :unidademedida");
        $stmt->bindParam(':unidademedida',$unidademedida);
        $stmt->execute();
         $rows = $stmt->rowCount();

          if ($rows <=0) {
          $stmt = $pdo->prepare("insert into unidademedidatcc (unidademedida) values (:unidademedida)");
          $stmt->bindParam(':unidademedida',$unidademedida);
           $stmt->execute();
         }
       }
 ...
 if ($_SERVER["REQUEST_METHOD"] == 'POST') {
  try{
    ...
    if ((trim($categoria) == "")) {
      ...
      include("conexaobanco.php");
        $stmt = $pdo->prepare("select * from categoriatcc where categoria = :categoria");
        $stmt->bindParam(':categoria',$categoria);
        $stmt->execute();
         $rows = $stmt->rowCount();

          if ($rows <=0) {
          $stmt = $pdo->prepare("insert into categoriatcc (categoria) values (:categoria)");
          $stmt->bindParam(':categoria',$categoria);
           $stmt->execute();
         }
       }
  ...
  if ($_SERVER["REQUEST_METHOD"] == 'POST') {
    try{
      ...
      if ((trim($nomeProduto) == "") || (trim($codigoProduto) == "")) {
      }else{
        include("conexaobanco.php");
        //verificando se produto ja existe 
        $stmt = $pdo->prepare("select * from produtotcc where nomeProduto = :nomeProduto");
        $stmt->bindParam(':nomeProduto',$nomeProduto);
        $stmt->execute();
         $rows = $stmt->rowCount();

        if ($rows <=0) {
          $stmt = $pdo->prepare("insert into produtotcc (nomeProduto,codigoProduto,categoriaProduto,unidadeProduto) values (:nomeProduto,:codigoProduto,:categoriaProduto,:unidadeProduto)");
          $stmt->bindParam(':nomeProduto',$nomeProduto);
          $stmt->bindParam(':codigoProduto',$codigoProduto);
          $stmt->bindParam(':categoriaProduto',$categoriaProduto);
          $stmt->bindParam(':unidadeProduto',$unidadeProduto);
          $stmt->execute();
...
<?php
  if($_SERVER["REQUEST_METHOD"] === 'POST'){
    try{
      ...
      if ((trim($nomePessoa) == "") || (trim($codigoPessoa) == "")) {
      
      }else{
        include("conexaobanco.php");
        //verificando se pessoa ja existe 
        $stmt = $pdo->prepare("select * from pessoatcc where nomePessoa = :nomePessoa");
        ...
        if ($rows <=0) {
          $stmt = $pdo->prepare("insert into pessoatcc (nomePessoa,codigoPessoa,tipoPessoa) values (:nomePessoa,:codigoPessoa,:tipoPessoa)");
          $stmt->bindParam(':nomePessoa',$nomePessoa);
